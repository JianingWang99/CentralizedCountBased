...
class RandomPolicy(Policy):
    def __init__(self, env, agent_index):
        ...
        self.agent_action_space = self.env.action_space[self.agent_index].shape[-1]

    def action(self, obs):
        # ignore observation and just act based on keyboard events
        if self.env.discrete_action_input:
            u = np.random.choice([i for i in range(4)])
        else:
            print(self.agent_action_space)
            u = np.zeros(self.agent_action_space) # 5-d because of no-move action
            m = np.random.choice([i for i in range(self.agent_action_space)])
            u[m]+= 1.0
        return np.concatenate([u, np.zeros(self.env.world.dim_c)])
